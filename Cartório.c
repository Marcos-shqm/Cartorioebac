...
#include <stdlib.h> //biblioteca de alcação de espaço em memória
...
int registro() //Função Responsável por cadastrar no sistema
{
    ...
    char sobrenome[40];
    char cargo[40];
    //Final de criação de variável/string
    
    printf("Digite o CPF a ser cadastrado: "); //Coletando informação do usuário
    scanf("%s", cpf); // %s refer-se o string
    
    strcpy(arquivo, cpf); //Responsável por copiar os valores das string
    
    FILE *file; // cria o arquivo
    file = fopen(arquivo, "w"); //cria o arquivo e o "w" significa escrever
    fprintf(file,cpf); //salva o arquivo da variável
    ...
    file = fopen(arquivo, "a"); //cria o arquivo e o significado...
    fprintf(file,nome); //salva o arquivo da variável
    fclose(file); //fecha  o arquivo
    
    file = fopen(arquivo, "a"); //cria o arquivo
    fprintf(file,","); //salva o arquivo 
    fclose(file); //fecha o arqquivo
    
    printf("Digite o sobrenome a ser cadastrado: "); //Coletando informações do usuário
    scanf("%s",sobrenome); // %s refere-se o string
    
    file = fopen(arquivo, "a"); //cria o arquivo eo significado
    ...
    scanf("%s",cargo); // refer-se o string
